...
el Transformer entrenado (clase SignTransformer, definida en comun.py).

Maquina de estados:
    IDLE        -> esperando que aparezca mano.
    DETECTANDO  -> hay mano; se acumulan frames y al llenar el buffer se corre
                   el modelo con Test-Time Augmentation.
    COMMIT      -> se muestra la prediccion ganadora antes de volver a IDLE.

Controles: ESC sale, R resetea buffer y estado.
"""
...
# Buffer mayor que MAX_SEQ_LEN: en vivo la camara corre mas rapido que los 25fps
# de grabacion y una senia larga supera los 80 frames; construir_tensor lo
# submuestrea a MAX_SEQ_LEN antes de pasarlo al modelo.
BUFFER_LEN = 80
...
MS_MANO_FUERA_RESET = 600    # ms sin mano en COMMIT para volver a capturar
...
def resolver_dir_modelo():
    """Carpeta del modelo a usar. Soporta el layout multi-modelo (subcarpetas
    de modelos/) y el layout antiguo (archivos sueltos en modelos/).

    Orden: SIGNAI_MODELO -> 'estatico' -> primera subcarpeta valida -> raiz."""
    archivos = ("signai_transformer.pth", "label_map.json", "feature_meta.json")
    pref = os.environ.get("SIGNAI_MODELO", "").strip()
    candidatos = []
    if pref:
        candidatos.append(DIR_MODELOS / pref)
    candidatos.append(DIR_MODELOS / "estatico")
    if DIR_MODELOS.exists():
        for sub in sorted(DIR_MODELOS.iterdir()):
            if sub.is_dir():
                candidatos.append(sub)
    candidatos.append(DIR_MODELOS)  # layout antiguo (archivos sueltos)
    for c in candidatos:
        if all((c / a).exists() for a in archivos):
            return c
    return DIR_MODELOS


def cargar_modelo(device):
    dir_modelo = resolver_dir_modelo()
    p_modelo = dir_modelo / "signai_transformer.pth"
    p_label = dir_modelo / "label_map.json"
    p_meta = dir_modelo / "feature_meta.json"
    if not p_modelo.exists():
        print(f"ERROR: no se encontro {p_modelo}.")
        sys.exit(1)
    if not p_label.exists():
        print(f"ERROR: no se encontro {p_label}")
        sys.exit(1)
    if not p_meta.exists():
        print(f"ERROR: no se encontro {p_meta}")
        sys.exit(1)
    print(f"[inferencia] usando modelo: {dir_modelo.name}")
...
def construir_tensor(buffer_frames_static):
    """buffer_frames_static: vectores [190] por frame.
...
def dibujar_hud(frame, estado, pred_txt, conf, num_manos, buffer_len, fps, top3):
    ...
    cv2.putText(frame, f"Manos: {num_manos}   Buffer: {buffer_len}/{BUFFER_LEN}",
    ...
    try:
        while True:
            ...
            if estado == IDLE:
                ...
                # Mano ausente antes de llenar el buffer: senia incompleta -> reset.
                if not hand_present:
                ...
                # La senia ya se predijo: se mantiene en pantalla y se espera a
                # que la mano se vaya antes de volver a capturar.
                if not hand_present:
                    if sin_mano_desde_ms is None:
                        sin_mano_desde_ms = ahora
                    if ahora - sin_mano_desde_ms > MS_MANO_FUERA_RESET:
                        estado = IDLE
                        buffer_static.clear()
                        texto_commit = ""
                        conf_commit = 0.0
